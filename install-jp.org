...
* 設定成功報告
- [2020-08-18 Tue]: Ubuntu 20.04.1 LTS, Coq 8.12.0 (opam), MathComp 1.12.0 (opam)
...
現時点[2020-08-18 Tue], ~opam show coq-mathcomp-ssreflect~ によると,
1.11.0版のパッケージがあります.

- 最初に, [[https://opam.ocaml.org/doc/Install.html][opamを設定]](バージョン2.0.7 [2020-08-18 Tue])します:
  ...
  + ~opam init~
    ...
    * コンパイラを指定できます; 例えば, ~opam init --comp=4.10.0~
    * ~eval $(opam env)~ で設定が完了します
  + opamは既に設定された場合, 最新のパッケージの情報を取得: ~opam update~
  + 使えるパッケージのリスト: ~opam list -a~
- 次に, OCamlの環境を設定します:
  + ~opam switch 4.10.0~
    * 初めての設定なら, ~opam switch create 4.10.0~
    * ~opam switch list~ からその他のOCamlのコンパイラを選べます. 現時点[2020-08-18 Tue], 4.10.0は最新です.
  + ~eval `opam config env`~
    * ~opam switch~ を行った後, 必要です.
  + 次のOCamlのプログラムはCoqに必要です(自分の環境の整備によって, 他のプログラムを設定する必要があるかもしれませんが, opamが教えます):
    * ~opam install camlp5~ (現時点[2020-08-18 Tue], 8.00 alphaは最新です)
    * ~opam install ocamlfind~ (現時点[2020-08-18 Tue], 1.8.1は最新です)
      - ~conf-m4~ に依存します
    * ~opam install num~ (現時点[2020-08-18 Tue], 1.3は最新です; Ocaml 4.06.0から必要です)
- 次に, Coqを設定します:
  ...
  + ~opam install coq.8.12.0~ (現時点[2020-08-18 Tue]最新版)
    * 必要なら, 最新のCoqIDEも設定できます: ~opam install coqide~
    * Debian/Ubuntuで ~libgtksourceview-3.0-dev~, ~libgtk-3-dev~,
      ~libexpat1-dev~, ~libcairo2-dev~ が要ります (~pkg-config~ は要らなくなりましたか)
- 最後に, MathCompを設定します:
...
- 今度の更新は, ~opam update~ , ~opam upgrade coq.8.12.0~ (例えば) で済みます.
