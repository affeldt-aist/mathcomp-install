...
- WSL上でEmacsを使いたいなら, Xorgサーバーを設定しなければなりません.
  まずは[[https://sourceforge.net/projects/vcxsrv/][VcxSrv]]をダウンロードしてインストールします. WSLのshellで
  ~DISPLAY~ という環境変数の設定も必要です.　Ubuntu 18.04の場合は,
  ~.bashrc~ に次のコードを追加します:
#+BEGIN_SRC
...
*** 方法2 (cygwin + opam): customなopamを用いて, Coqをコンパイルします
...
*** 方法3 (WSL + opam): opamを用いて, Coqをコンパイルします
